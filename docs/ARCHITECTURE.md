...
matches/{matchId}
{
  ...
  players: {
    p1: {
      ...
      hp:      100,
      score:   0,
      weapon:  "lightsaber" | "greatsword" | "smallsword",  // 選択中の武器（プレイヤーごと、iPhone が書く）
    ...
    p2: {
      ...
      hp:      100,
      score:   0,
      weapon:  "lightsaber" | "greatsword" | "smallsword",
      drawn:   false,
...
> 武器選択は **プレイヤーごと**（`players.pX.weapon`）で確定。試合単位の `weapon` は持たない。
> 現状の iOS は Firebase SDK を使わず REST API (`PATCH .../documents/matches/{matchId}`) で `players.pX` の上記フィールドを直接更新する（[ios-app/TAKI.md の Firestore 連携セクション参照](../ios-app/TAKI.md)）。
...
| フィールド | 書く人 | 読む人 | 意味 |
|------------|--------|--------|------|
| `players.pX.ready` / `readyAt` | iOS（たき・REST 直叩き暫定） | 大画面（みずき） | 正しく構えられた／その時刻 |
| `players.pX.drawn` / `drawnAt` | iOS（たき・REST 直叩き暫定） | 大画面（みずき） | 抜刀完了／その時刻 |
| `players.pX.weapon` | iOS（たき） | 大画面（みずき） | 選択中の武器 |
...
- [x] 武器はプレイヤー共通か、各自で選ぶか → **各自選択**で確定（`players.pX.weapon` を iOS が書く）
