...
| フィールド | 書く人 | 読む人 | 意味 |
|------------|--------|--------|------|
| `players.pX.ready` | iPhone（たき） | 大画面（みずき） | 正しく構えられた |
| `players.pX.drawn` | iPhone（たき） | 大画面（みずき） | 抜刀完了 |
| `players.pX.score` | PC カメラ（はる） | 大画面（みずき） | 斬撃の累計 |
| `players.pX.hp` | バトルロジック（はる） | 大画面（みずき） | 残り HP |
| `status` | 進行役（はる or たき） | 全員 | 試合フェーズ |
...
1. **たき**：iPhone から `matches/test/players/p1/drawn` を `true` に更新できる。
